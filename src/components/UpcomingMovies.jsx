...
export default function UpcomingMovies({ onAddWatchlist }) {
  const [movies, setMovies] = useState([]);
  const [selectedMovie, setSelectedMovie] = useState(null);

  const API_URL =
    "https://api.themoviedb.org/3/movie/upcoming?api_key=7d9969ab4156d9f0eb56a1e38298be66";
  ...
  if (!movies.length || !selectedMovie)
    return <div className="loading">Loading Upcoming Movies...</div>;
      ...
      <div className="upcoming-movie-list">
        {movies.map((movie) => (
          <div
            ...
            aria-label={`Select upcoming movie: ${movie.title}`}
            onKeyDown={(e) => {
